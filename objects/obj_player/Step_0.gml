...
or sprite_index == spr_tumble or (state == states.fireass) or (state == states.firemouth) or (state == states.hookshot) or (state == states.skateboard) or  (state = states.mach4) or (state == states.Sjump) or (state == states.machroll) or (state == states.machfreefall) or (state == states.tacklecharge)  or (state == states.superslam && sprite_index = spr_piledriver) or (state == states.knightpep) or (state == states.knightpepattack) or (state == states.knightpepslopes)  or (state == states.boxxedpep) or (state == states.cheesepep) or (state == states.cheeseball) or (state == states.slipbanan) or (state == states.spindash) or (state == states.handstandjump && (sprite_index == spr_attackdash or sprite_index == spr_airattackstart or sprite_index == spr_airattack)) or state == states.chainsawbump or state == states.rocket
or state == states.punch or (state == states.ratmountbounce && vsp >= 0) or sprite_index == spr_player_Sjumpcancel or (sprite_index == spr_swingding && global.gameplay != 0) or state == states.boxxedpepspin or state == states.trashroll or state == states.trashjump
	instakillmove = true
else
	instakillmove = false
